...
def bounded(a:float, b:float, c:float) -> float:
    assert 0 < a < 1, f"forbidden value for bounded analogy : {round(a, 2)}:{round(b, 2)}::{round(c, 2)}:?"
    assert 0 < b < 1, f"forbidden value for bounded analogy : {round(a, 2)}:{round(b, 2)}::{round(c, 2)}:?"
    assert 0 < c < 1, f"forbidden value for bounded analogy : {round(a, 2)}:{round(b, 2)}::{round(c, 2)}:?"
